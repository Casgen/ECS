...
public:
  Camera(const glm::vec3 &position, const float &aspectRatio,
         const glm::ivec2 &resolution, const float &a = 0, const float &z = 0);
  ~Camera();
  ...
  void DecMovementSpeed(const float &num) {
    ...
    m_SlopeInc =
        glm::max(m_SlopeInc - (num * m_SlopeInc / m_MovementSpeed), 1e-4f);
  }

  template <typename NumType> void Yaw(const NumType &step);

  template <typename NumType> void Pitch(const NumType &step);
  // ---------------------------------------------------------
...
private:
  ...
  // Used for the ease-in-out movement of the camera (how much speed will the
  // camera be picking up through time);
...
/**
 *  A struct with a bit field to identify in which direction is the camera
 * moving.
 */
struct MovingBitField {
  bool isFreeMode : 1, isMoving : 1, isFwd : 1, isLeft : 1, isBack : 1,
      isRight : 1, isUp : 1, isDown : 1;

  /**
   * Constructs a bit field;
   * @param isFree - Sets a Free mode boolean to indicate that the camera can
   * freely move.
   */
  ...
  void CheckIsMoving() {
    isMoving = isFwd || isLeft || isBack || isRight || isUp || isDown;
  }
};
